...
# -*- coding: utf-8 -*-
"""
Created on Thu May 12 13:21:06 2022
...
class detector():
    ...
    def distanceFromPixels(self,img,centerList,camheight):
        ...
        #finds the distances
        distanceListX = np.array([])
        distanceListY = np.array([])
        for box in centerList:
            xdif = abs(box[0]-imgx/2)
            ydif = abs(box[1]-imgy/2)
            x = xdif*plen*h/f
            y = ydif*plen*h/f
            np.append(distanceListX, x) #Her har vi gjort mye rot
            np.append(distanceListY, y)
            
        distanceList = np.array([distanceListX, distanceListY])
        return distanceListX
    
    def makeCartesianList(self, img, centerlist):
        roecoordinates = np.zeros((len(centerlist), 2))
        xori = int(img.shape[0]/2) 
        yori = int(img.shape[1]/2)
        for pixel in range(len(centerlist)):
            xpix = centerlist[pixel][0]
            ypix = centerlist[pixel][1]
            x = ((xpix - xori)/xori)*15
            y = ((ypix - yori)/yori)*15
            roecoordinates[pixel][0] = x
